-- 003-add-amux-channel.sql
-- Purpose: add amux (api.amux.ai) Gemini image channel (type 64).
--
-- Models:
...
--
-- Key: PLACEHOLDER_AMUX_KEY — fill in via admin console after apply.
-- Auth: Bearer token (set automatically by amux adaptor).
...
SELECT
  ...
  1,
  'https://api.amux.ai',
  'PLACEHOLDER_AMUX_KEY',
  EXTRACT(EPOCH FROM NOW())::bigint, 0,
  10, 100, 'amux',
  NULL, NULL, NULL
WHERE NOT EXISTS (
  SELECT 1 FROM channels WHERE name = 'amux' AND type = 64 AND "group" = 'default'
);
...
WHERE name = 'amux' AND type = 64 AND "group" = 'default';

-- ── Step 2: seed abilities ────────────────────────────────────────────────────

INSERT INTO abilities ("group", model, channel_id, enabled, priority, weight, tag)
SELECT g.grp, m.model, c.id, true, 10, 100, 'amux'
FROM (VALUES
  ('gemini-2.5-flash-image'),
  ('gemini-3-pro-image-preview'),
  ('gemini-3.1-flash-image-preview'),
...
JOIN channels AS c
  ON c.name = 'amux' AND c.type = 64 AND c."group" = 'default'
