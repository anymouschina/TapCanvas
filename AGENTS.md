...
- 本项目的唯一质量目标是可持续商业运营的生产级应用，不是演示项目、原型项目或“先凑合能用”的临时系统。功能设计、技术方案、数据结构、交互体验与运维能力都必须以长期运营、真实用户和真实资金成本为前提。
- 每次改动必须先明确业务目标、领域边界、上下游契约、数据一致性要求和运营影响，再选择实现路径。禁止只针对当前页面、当前报错或当前测试样例堆叠局部补丁。
- 必须修复根因。禁止通过重复分支、临时适配层、隐式兜底、静默降级、复制粘贴或绕过既有契约来掩盖架构问题；若现有职责边界无法承载新能力，应先完成必要的模块拆分或契约重构。
- 商业功能必须同时考虑正确性、安全性、多用户数据隔离、权限控制、计费与积分一致性、幂等性、并发安全、审计日志、可观测性、性能、可维护性以及部署回滚能力，不得只验证理想路径。
- 所有失败必须显式、可定位、可追溯。关键链路必须保留足够的结构化日志与事实证据，禁止以默认值、假数据、吞异常或前端伪成功状态换取表面可用。
- 改动完成后必须执行与风险匹配的类型检查、构建、自动化测试和受影响流程回归；涉及计费、积分、支付、权限、模型调用、数据写入与并发控制时，必须覆盖成功、失败、重复请求和边界场景。
- “本地能跑”不等于完成。交付前必须显式 review：对照原始需求、实施计划、实际 diff、前后端契约、数据库影响、文档同步和测试证据逐项核验；发现缺口必须继续修复，不得以待办或口头说明代替当前应完成的工作。
- 形成一个职责单一、行为完整、验证通过、可独立回滚的合适版本后，必须及时提交 Git。每个提交只包含同一交付意图的相关文件，使用 Conventional Commits，禁止混入无关改动、未验证代码、构建产物、密钥或本地临时文件。
- 用户已明确授权按上述门槛对合适版本执行 `git add` 与 `git commit`；提交前仍必须核对工作区和暂存区范围。该授权不包含 `push`、`pull`、`merge`、`rebase`、分支或标签操作，这些操作仍需用户当次明确同意。
